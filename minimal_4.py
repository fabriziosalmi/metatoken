...
# --- 1. CONFIGURAZIONE E IPERPARAMETRI V4 ---
block_size = 32      
batch_size = 4       
max_iters = 5000     
eval_interval = 250  
...
# ### MODIFICA V4: Ridimensioniamo il modello per combattere l'overfitting ###
d_model = 96        # Era 128
n_head = 6          # Era 8
n_layer = 6         # Era 8
dropout = 0.2       # ### MODIFICA V4: Aggiungiamo il dropout! ###
...
# --- 2. TOKENIZER INNOVATIVO "SMART" (Invariato) ---
...
# --- 3. GESTIONE DATI DA FILE ESTERNO (Invariato) ---
...
# Usiamo l'ultimo esempio per la validazione, il resto per il training
...
# --- 5. ARCHITETTURA DEL MODELLO V4 (CON REGOLARIZZAZIONE) ---
class Block(nn.Module):
    def __init__(self, n_embd, n_head):
        ...
        self.ffwd = nn.Sequential(
            ...
            nn.Dropout(dropout), # ### MODIFICA V4: Dropout nella feed-forward ###
        )
...
class MetaWordTransformer(nn.Module):
    def __init__(self):
        ...
        # ### MODIFICA V4: Dropout dopo la somma degli embedding ###
    ...
    def forward(self, word_idx, ruolo_idx, semantico_idx, targets_w=None, targets_r=None, targets_s=None):
        ...
        x = self.emb_dropout(x) # ### MODIFICA V4: Applichiamo il dropout ###
        x = self.blocks(x)
        x = self.ln_f(x)
...
m = model.to(device)
# ### MODIFICA V4: Aggiungiamo weight_decay all'ottimizzatore ###
optimizer = torch.optim.AdamW(m.parameters(), lr=learning_rate, weight_decay=1e-1)
