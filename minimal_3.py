...
import re
import json # Importiamo la libreria per leggere i file JSON

# --- 1. CONFIGURAZIONE E IPERPARAMETRI (Invariati da V2) ---
block_size = 32      
batch_size = 4       
max_iters = 5000     
eval_interval = 250  
...
DATASET_FILE = 'dataset.json' # Nome del file da caricare
# ----------------------------------------------------

print(f"Sto usando il device: {device}")

# --- 2. TOKENIZER INNOVATIVO "SMART" (Invariato da V2) ---
...
# --- 3. GESTIONE DATI DA FILE ESTERNO ---
def load_data_from_json(filepath):
    """Carica i dati dal file JSON e li converte nel formato interno."""
    try:
        with open(filepath, 'r', encoding='utf-8') as f:
            data = json.load(f)
    except FileNotFoundError:
        print(f"ERRORE: Il file '{filepath}' non è stato trovato. Assicurati che sia nella stessa cartella dello script.")
        exit()

    # Convertiamo la struttura da dizionari a tuple, come si aspetta il resto del codice
    processed_data = []
    for example in data['examples']:
        seq = []
        for token_data in example['sequence']:
            seq.append((token_data['word'], token_data['ruolo'], token_data['semantico']))
...
# Dividiamo i dati: usiamo l'ultimo esempio per la validazione
train_data = raw_data[:-1]
val_data = raw_data[-1:] # Deve essere una lista, quindi usiamo lo slicing

# Creiamo i vocabolari (logica invariata)
...
# Funzione get_batch (invariata da V2)
def get_batch(split):
    data = train_data if split == 'train' else val_data
    if not data: # Controllo per evitare errori se il set di validazione è vuoto
        print(f"Attenzione: il set di dati '{split}' è vuoto.")
        return None
    
    # Se il dataset di validazione ha meno esempi del batch_size, campioniamo con ripetizione
    ...
    for seq in seqs:
        ...
        def pad(l, size, pad_id): return l + [pad_id] * (size - len(l))
...
# --- Sezioni 4, 5, 6 (METRICHE, MODELLO, TRAINING) sono INVARIATE da V2 ---
...
for iter in range(max_iters):
    if iter % eval_interval == 0 or iter == max_iters - 1:
        ...
        if 'val_acc_word' in metrics: print(f"  val acc word: {metrics['val_acc_word']:.2%}, ruolo: {metrics['val_acc_ruolo']:.2%}, semantico: {metrics['val_acc_semantico']:.2%}")

    batch_train = get_batch('train')
    if batch_train:
        xb_w, xb_r, xb_s, yb_w, yb_r, yb_s = batch_train
