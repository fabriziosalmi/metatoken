...
# --- 1. CONFIGURAZIONE E IPERPARAMETRI V5 ---
block_size = 48      # Increased to fit longest sequences (max 46 tokens)
batch_size = 4       
max_iters = 5000     
eval_interval = 250  
learning_rate = 5e-4 # Aumentato leggermente per lo scheduler
...
n_head = 6          
n_layer = 6         
dropout = 0.3       # Dropout più aggressivo
...
# --- 4. CALCOLO METRICHE (MODIFICATO PER TARGET SINGOLO) ---
@torch.no_grad()
def estimate_metrics():
    ...
    for split in ['train', 'val']:
        ...
        for k in range(eval_iters):
            Xw, Xr, Xs, Yw, _, _ = get_batch(split) # Ignoriamo Yr e Ys
...
# --- 5. ARCHITETTURA DEL MODELLO V5 (TARGET SEMPLIFICATO) ---
class Block(nn.Module):
    def __init__(self, n_embd, n_head):
        super().__init__()
        self.sa = nn.MultiheadAttention(n_embd, n_head, dropout=0.1, batch_first=True)
        self.ffwd = nn.Sequential(nn.Linear(n_embd, 4 * n_embd), nn.ReLU(), nn.Linear(4 * n_embd, n_embd), nn.Dropout(dropout))
...
class MetaWordTransformer(nn.Module):
    ...
    def generate(self, word_idx, ruolo_idx, semantico_idx, max_new_tokens, top_p):
        for _ in range(max_new_tokens):
            ...
            # Dobbiamo estendere anche i meta-token per il prossimo ciclo, usando l'ultimo valore noto
...
m = model.to(device)
optimizer = torch.optim.AdamW(m.parameters(), lr=learning_rate, weight_decay=5e-1) # Weight decay più forte
scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=max_iters)

# Calcoliamo i parametri solo per le parti rilevanti del modello V5
...
start_ruolo, start_semantico = 'UTENTE', 'AZIONE' # I meta-token sono ancora cruciali come input!
